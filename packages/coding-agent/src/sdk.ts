...
async function createAgentSessionScoped(options: CreateAgentSessionOptions): Promise<CreateAgentSessionResult> {
	...
	try {
		...
		// The native Jev tool router (default disabled) resolves through the
		...
		const scopedBoundStreamFn = (scope: ToolRouterScope): StreamFn =>
		...
		const withSessionRedaction =
			(inner: StreamFn): StreamFn =>
			(streamModel, context, streamOptions) =>
				withCredentialRedaction(cfgSecretsEnabled.get(settings), () => inner(streamModel, context, streamOptions));
		const scopedSettingsAwareStreamFn = (scope: ToolRouterScope): StreamFn =>
			withSessionRedaction(scopedBoundStreamFn(scope));
		...
		// Shared by main and capture so both keep the identical upstream policy.
		const withPrimaryProviderOptions =
			(inner: StreamFn): StreamFn =>
			(streamModel, context, streamOptions) => {
				const kimiApiFormat = cfgProvidersKimiApiFormat.get(settings);
				return inner(streamModel, context, {
					...streamOptions,
		...
		session = new AgentSession({
			...
			onPayload,
			onResponse,
			sideStreamFn: scopedSettingsAwareStreamFn("side-channel"),
			advisorStreamFn: scopedSettingsAwareStreamFn("advisor"),
			convertToLlm: convertToLlmFinal,
			rebuildSystemPrompt,
